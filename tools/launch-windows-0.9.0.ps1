...
$fallbackStatePath = Join-Path $dataDirectory 'windows-090-launch-fallback.json'
...
if (-not $attachedToExistingShim) {
    if ($null -ne (Get-ShimProcess)) {
        ...
        $backupDirectory = Join-Path $backupRoot "env-090-launch-$stamp"
