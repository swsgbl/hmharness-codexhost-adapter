...
$fallbackStatePath = Join-Path $dataDirectory 'windows-091-launch-fallback.json'
...
if (-not $attachedToExistingShim) {
    if ($null -ne (Get-ShimProcess)) {
        ...
        $backupDirectory = Join-Path $backupRoot "env-091-launch-$stamp"
