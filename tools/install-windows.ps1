...
# Run scripts/apply-codexhost-0.9.1.mjs first on a clean installation. That
...
$pluginSource = Join-Path $PSScriptRoot '..\snapshots\codex-host-runtime\0.9.1\plugin'
...
if ($distribution.version -ne '0.9.1') {
    throw "This artifact is verified only with CodexHost 0.9.1; found $($distribution.version)."
}
...
if (Test-Path -LiteralPath $bundledPlugin -PathType Container) {
    ...
    $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $bundledBackup = Join-Path $BackupRoot "codexhost-091-hmharness-plugin-$stamp"
