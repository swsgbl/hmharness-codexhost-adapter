...
# Run scripts/apply-codexhost-0.10.0.mjs first on a clean installation. That
...
$pluginSource = Join-Path $PSScriptRoot '..\snapshots\codex-host-runtime\0.10.0\plugin'
...
if ($distribution.version -ne '0.10.0') {
    throw "This artifact is verified only with CodexHost 0.10.0; found $($distribution.version)."
}
...
if (Test-Path -LiteralPath $bundledPlugin -PathType Container) {
    ...
    $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $bundledBackup = Join-Path $BackupRoot "codexhost-0100-hmharness-plugin-$stamp"
