...
1. `@hmharness/codexhost-bridge@0.6.1` is public on the official npm registry with MIT metadata, no `private` flag, and the correct `repository.directory`.
2. A clean project installed the official-registry tarball and verified `--version`, provider listing, and credential-free output. `--version` returned `{"version":"0.6.1"}`, the isolated provider listing returned `alt-model`, and neither fake key appeared.
...
5. Earlier integration validation covered TypeScript build, typecheck, lint, 256 targeted tests, a real HMHarness task, bridge process exit, and zero Thinking indicators after completion.
...
1. `@hmharness/codexhost-bridge@0.6.1` 已在 npm 官方源公开，MIT 元数据正确，没有 `private` 标记，`repository.directory` 指向正确。
2. 已在干净项目中安装官方源 tarball，并完成 `--version`、provider 列表和无凭据输出验证。`--version` 返回 `{"version":"0.6.1"}`，隔离 provider 列表返回 `alt-model`，两个假 key 均未出现。
...
5. 此前集成验证已覆盖 TypeScript 构建、typecheck、lint、256 个定向测试、真实 HMHarness 任务、bridge 进程退出，以及完成后 Thinking 指示为 0。
