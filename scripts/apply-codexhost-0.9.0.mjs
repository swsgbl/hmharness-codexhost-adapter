...
const expectedCodexHostVersion = "0.9.0";
...
function replaceOnce(source, search, replacement, label) {
  ...
  if (count !== 1) {
    throw new Error(`Expected one CodexHost 0.9.0 patch anchor for ${label}, found ${count}`);
  }
...
function replaceExactCount(source, search, replacement, expectedCount, label) {
  ...
  if (count !== expectedCount) {
    throw new Error(`Expected ${expectedCount} CodexHost 0.9.0 patch anchors for ${label}, found ${count}`);
  }
